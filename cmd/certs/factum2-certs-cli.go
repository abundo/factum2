...
func main() {
	...
	cmdbase.Run(boa.CmdT[Params]{
		Use:     "factum2-certs",
		Short:   "Manage ACME certificates via lego",
		Version: buildinfo.Version,

		SubCmds: boa.SubCmds(
			cmdbase.ShowConfigAgent(func(fc *util.ConfigFactum) (any, error) { return certs.FetchRemoteConfig(fc) }),
			boa.CmdT[SyncParams]{
				Use:   "sync",
				Short: "Write .lego.yaml / .env and run lego",
				RunFuncE: func(p *SyncParams, cmd *cobra.Command, args []string) error {
					cmdbase.SetupLog(p.CommonParams)
					d, err := certs.NewClient(&p.Config)
					if err != nil {
						return err
					}
