// Package netboxtool is Factum's NetBox GraphQL/REST client (devices, VMs,
// interfaces, addresses, cables, VRFs, L2VPNs). Reads go through GraphQL;
// writes go through REST. See README.md in this directory for query
// architecture and pagination notes.
package netboxtool
...
// vlanNamesFromRefs collects VID -> name from the nested GraphQL vlan
// objects on one interface. Empty names are skipped (Netbox always has a
// name, but a truncated/partial payload shouldn't invent one). Returns nil
// when nothing resolved, so callers can leave VLANNames unset.
func vlanNamesFromRefs(untagged, qinq *NetboxVlanRef, tagged []NetboxVlanRef) map[int]string {
	out := map[int]string{}
	add := func(ref *NetboxVlanRef) {
		if ref == nil || ref.VID == 0 || ref.Name == "" {
			return
		}
		out[ref.VID] = ref.Name
	}
	add(untagged)
	add(qinq)
	for i := range tagged {
...
func (nb *NetboxClient) parseDevices(devices []JSONDevice, vm bool) ([]*NBDevice, error) {
	...
	for _, device := range devices {
		...
		for _, jintf := range device.Interfaces {
			...
			iface.VLANNames = vlanNamesFromRefs(jintf.UntaggedVLAN, jintf.QinQSVlan, jintf.TaggedVLANs)
...
// RestGet, RestPost and RestPatch expose the REST helpers for callers that
// talk to extras endpoints this package does not wrap (custom fields,
// webhooks, event rules).
func (nb *NetboxClient) RestGet(endpoint string, out any) error {
	return nb.restGet(endpoint, out)
}

func (nb *NetboxClient) RestPost(endpoint string, payload, out any) error {
	return nb.restPost(endpoint, payload, out)
}
...
// GetSiteBySlug looks up a site by slug, including "Default". Returns
// nil, nil if none matches.
func (nb *NetboxClient) GetSiteBySlug(slug string) (*NetboxNamedRef, error) {
	return nb.restGetNamed("/api/dcim/sites/?slug=" + url.QueryEscape(slug))
}

// NetboxSiteREST is a dcim.Site row as returned by the REST API (plain
// JSON numbers for id/latitude/longitude, unlike GraphQL NetboxSite).
...
// CreateSite creates a Netbox site. Status defaults to "active"; extra may
// set latitude/longitude and any other writable site field. Name and slug
// must be unique.
func (nb *NetboxClient) CreateSite(name, slug string, extra map[string]any) (*NetboxSiteREST, error) {
	...
	var result NetboxSiteREST
	if err := nb.restPost("/api/dcim/sites/", payload, &result); err != nil {
		return nil, err
	}
	return &result, nil
}

// UpdateSite patches a Netbox site with arbitrary writable fields
// (latitude/longitude, name, slug, …).
func (nb *NetboxClient) UpdateSite(siteID uint, changes map[string]any) error {
	return nb.restPatch("/api/dcim/sites/"+strconv.FormatUint(uint64(siteID), 10)+"/", changes)
}
...
// fields (e.g. "type": "lag", "vrf": {"name": ...}) - for callers that
// know more about the interface being created than InterfaceCreate's
// fixed "virtual" type allows for.
...
// address/status, this is for callers that also set VRF and role (Netbox
// models those on the address, not the interface).
...
// matching a two-interface connection model.
...
// nil, nil (a two-interface connection model cannot represent those).
...
func (nb *NetboxClient) CreateCable(aInterfaceID, bInterfaceID uint) (*NBCable, error) {
	return nb.CreateCableWithOptions(aInterfaceID, bInterfaceID, nil)
}

// CreateCableWithOptions is CreateCable plus arbitrary extra REST fields
// (e.g. "label": "uplink"). extra may be nil.
...
func (v *netboxVRFREST) toNBVRF() *NBVRF {
	return &NBVRF{
		NetboxID:    v.ID,
		Name:        v.Name,
		RD:          v.RD,
...
// GetVRFByName looks up an existing VRF by its exact name, returning nil
...
func (nb *NetboxClient) GetVRFByName(name string) (*NBVRF, error) {
	...
	var page struct {
		Results []netboxVRFREST `json:"results"`
	}
	if err := nb.restGet("/api/ipam/vrfs/?name="+url.QueryEscape(name), &page); err != nil {
		return nil, err
	}
	if len(page.Results) == 0 {
		return nil, nil
	}
	return page.Results[0].toNBVRF(), nil
}

// CreateVRF creates a VRF. rd is omitted when empty (Netbox treats it as
// optional).
func (nb *NetboxClient) CreateVRF(name, rd, description string) (*NBVRF, error) {
	...
	var result netboxVRFREST
	if err := nb.restPost("/api/ipam/vrfs/", payload, &result); err != nil {
		return nil, err
	}
	return result.toNBVRF(), nil
}
...
// (pseudowire ID), returning nil (no error) if none exists. Fallback
// when the on-device name doesn't match the Netbox L2VPN name but the
// pseudowire ID does.
