...
In-tree NetBox GraphQL/REST client used by `internal/netbox`, device-sync,
drivers, and the web GUI. This used to be the standalone
`github.com/abundo/netboxtool` module; change the client here rather than
depending on a tagged library.

The standalone `netboxtool` CLI is not shipped. Diagnostic fetch-strategy
notes below still apply to `GetDevices`/`GetVMs`.
...
- Flat fetch-and-stitch for the small to-one lookup tables
  ...
  measured benefit once (2) above was fixed. This is a _different_ scope
...
actual bottleneck in a real full-table sync — not because of N+1 _REST_
...
and benchmark tooling lived in the old standalone `netboxtool` CLI
(`cmd/benchmark.go`: seven fetch-strategy variants, labeled A–G, with
per-phase timing and page counts; `cmd/introspect.go`'s
`introspect-ip-address` subcommand: read-only GraphQL schema introspection,
used once to find `assigned_object`'s real union member types instead of
guessing them — see below).
...
- Large: Netbox 4.5.9 in Docker, 2 CPUs, 6 granian workers, 1,375 devices /
  ...
  A real end-to-end sync (fetch + local DB writes + cable/site/
...
- A fully flat _REST_ variant (`/api/dcim/devices/`, `/api/dcim/
...
- On the large instance, the REST interfaces call appeared to leave the
  server measurably degraded for the _next_ request too: a GraphQL
...
- [**#22787**](https://github.com/netbox-community/netbox/issues/22787) —
  ...
  maintainer as likely affecting _every_ GFK-backed GraphQL field in
...
- [**#22813**](https://github.com/netbox-community/netbox/issues/22813) —
  requesting `custom_fields` on _any_ GraphQL list endpoint cost one extra
...
- [**#22837**](https://github.com/netbox-community/netbox/issues/22837) —
  ...
  cost one row for the related object; it costs one row for _every other
  object that shares that same related object_, independent of page size
...
Combined effect of this session's client-side rework _and_ the Netbox
...
flat+concurrent approach is _still_ worth **~7.25x** (45.38s → 6.261s) —
...
schema, real-world timing) still means talking to a real (or lab) NetBox
instance — for example `factum2-netbox sync` against `dev/`.
