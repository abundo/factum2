...
watch(
  () => props.node || props.nodeName || null,
  (want) => {
    if (want) {
      loadShownNode()
    }
...
<template>
  ...
  <div v-else class="flex flex-col gap-3 min-h-0 h-full">
    <div class="flex flex-wrap gap-2 items-center shrink-0">
      ...
      <span v-if="shown?.group && shown.group !== 'default'" class="text-sm text-muted-color">
        {{ shown.group }}
      </span>
    </div>
    ...
    <UTabs v-model="tab" :items="tabItems" class="min-h-0 flex-1">
      <template #config>
        <div class="flex flex-col gap-2 pt-3 min-h-0">
          ...
          <pre
            v-else
            class="font-mono text-xs overflow-auto max-h-[calc(90vh-260px)] p-3 rounded-md bg-muted whitespace-pre"
      ...
      <template #versions>
        <div class="flex flex-col gap-3 pt-3 min-h-0">
          ...
          <UAlert v-else-if="versionsError" color="error" variant="subtle" :title="versionsError" />
          <template v-else>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 min-h-0">
              <div class="min-h-0">
                ...
                <pre
                  v-else
                  class="font-mono text-xs overflow-auto max-h-[calc(90vh-470px)] p-3 rounded-md bg-muted whitespace-pre"
              ...
              <div class="min-h-0">
                ...
                <UAlert v-else-if="diffError" color="error" variant="subtle" :title="diffError" />
                <div v-else-if="diffLines.length === 0" class="text-muted-color text-sm p-3">
                  No diff available.
                </div>
                <pre
                  v-else
                  class="font-mono text-xs overflow-auto max-h-[calc(90vh-470px)] p-3 rounded-md bg-muted"
                ><span
